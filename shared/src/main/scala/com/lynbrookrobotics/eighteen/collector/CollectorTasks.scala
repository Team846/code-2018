...
import com.lynbrookrobotics.eighteen.collector.clamp.{OpenCollector, CollectorClamp}
...
object CollectorTasks {
  ...
  def collectCubeWithoutOpen(rollers: CollectorRollers)
                            (implicit collectorRollersProps: Signal[CollectorRollersProperties]): ContinuousTask = {
    new SpinForCollect(rollers)
  }

  def purgeCube(rollers: CollectorRollers)
               (implicit collectorRollersProps: Signal[CollectorRollersProperties]): ContinuousTask = {
