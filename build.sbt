...
val crossCompileSettings = Seq(
  ...
  nativeCompileOptions ++= Seq(
    ...
    "-mfpu=neon",
    "-mfloat-abi=soft",
    "--sysroot=${CrossSettings.toolchainPath.abs}",
    s"-I${(libunwindFolder / "include").abs}",
    s"-I${(librtFolder / "include").abs}",
    s"-I${(boehmFolder / "include").abs}",
    "-I${CrossSettings.toolchainPath.abs}/include/c++/5.5.0",
    "-I${CrossSettings.toolchainPath.abs}/include/c++/5.5.0/arm-frc-linux-gnueabi",
